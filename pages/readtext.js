...
export default function ReadTextPage() {
  ...
  // Handle highlighting when highlight parameter changes
  useEffect(() => {
    if (highlight && contentRef.current) {
      const content =
        textData?.type === 'text'
          ? textData.content
          : textData?.processingResult || '';
      ...
      if (content) {
        ...
        if (position >= 0 && position < words.length) {
          ...
          const highlightedText = words
            .map((word, index) => {
              if (index === position) {
                return `<span style="background-color: yellow; padding: 2px;">${word}</span>`;
              }
              return word;
            })
            .join(' ');
          ...
          const highlightedElement = contentRef.current.querySelector('span');
